    ...
    class App
      module Support
        module SchedulerTask
          ...
          class CleanExpiredWorkspaces

            def initialize scheduler
              
              @config = scheduler.config
              @logger = scheduler.logger.class.new(STDOUT)
              @logger.progname = "#{scheduler.logger.progname} -- TASK CleanExpiredWorkspaces"
            ...
            def run!
              ...
              Dir.foreach @workspace_path do |file|
                ...
                next unless File.exists? build_file

                if File.mtime(build_file) < (Time.now - (@config['workspace_expiration']))
                  FileUtils.rm_rf workspace_file
                  @logger.debug "Expired workspace \##{file} -- Deleted #{workspace_file}"
                end
